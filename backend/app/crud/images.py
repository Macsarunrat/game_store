...
async def save_fimename(db:AsyncSession,filename:str,is_main: bool,game_id: int):
    if is_main == True:
        # Fetch the old main image path to delete the physical file
        fetch_old_main = text('SELECT image FROM image WHERE game_id=:game_id AND is_main= True')
        old_main_records = (await db.exec(fetch_old_main, params={'game_id': game_id})).mappings().all()
        for record in old_main_records:
            old_path = record['image']
            if old_path and os.path.exists(old_path):
                try:
                    os.remove(old_path)
                except Exception as e:
                    print(f"Error removing physical file {old_path}: {e}")
